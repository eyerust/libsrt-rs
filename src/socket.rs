...
impl SrtSocket {
    ...
    pub fn bind<A: ToSocketAddrs>(self, addrs: A) -> Result<Self> {
        if let Ok(mut addrs) = addrs.to_socket_addrs() {
            if let Some(addr) = addrs.next() {
                ...
                let result = unsafe { srt::srt_bind(self.id, os_addr.as_ptr() as *const sockaddr, os_addr.len() as i32) };
    ...
    pub fn connect<A: ToSocketAddrs>(&self, addrs: A) -> Result<()> {
        ...
        let result = unsafe { srt::srt_connect(self.id, os_target.as_ptr() as *const sockaddr, os_target.len() as i32) };
...
impl SrtSocket {
    pub fn local_addr(&self) -> Result<SocketAddr> {
        ...
        let result = unsafe { srt::srt_getsockname(self.id, addr.as_mut_ptr() as *mut sockaddr, &mut addrlen as *mut c_int) };
        if result == -1 {
    ...
    pub fn peer_addr(&self) -> Result<SocketAddr> {
        let mut addr = OsSocketAddr::new();
        let mut addrlen: c_int = addr.capacity() as i32;
        let result = unsafe { srt::srt_getpeername(self.id, addr.as_mut_ptr() as *mut sockaddr, &mut addrlen as *mut c_int) };
        if result == -1 {
            error::handle_result("0.0.0.0:0".parse().unwrap(), result)
        } else {
    ...
    pub fn accept(&self) -> Result<(Self, SocketAddr)> {
        ...
        let result = unsafe { srt::srt_accept(self.id, addr.as_mut_ptr() as *mut sockaddr, &mut _addrlen as *mut c_int) };
        if result == -1 {
    ...
    pub fn send(&self, buf: &[u8]) -> Result<usize> {
        let result = unsafe { srt::srt_send(self.id, buf as *const [u8] as *const c_char, buf.len() as i32) };
        if result == -1 {
    ...
    pub fn recv(&self, buf: &mut [u8]) -> Result<usize> {
        let result = unsafe { srt::srt_recv(self.id, buf as *mut [u8] as *mut c_char, buf.len() as i32) };
        if result == -1 {
            error::handle_result(result as usize, result)
        } else {
            Ok(result as usize)
    ...
    pub fn get_sender_buffer(&self) -> Result<(usize, usize)> {
        let mut blocks = 0;
        let mut bytes = 0;
        let result = unsafe { srt::srt_getsndbuffer(self.id, &mut blocks as *mut usize, &mut bytes as *mut usize) };
        if result == -1 {
    ...
    pub fn get_events(&self) -> Result<srt::SRT_EPOLL_OPT> {
        ...
        error::handle_result(srt::SRT_EPOLL_OPT(events.try_into().expect("invalid events")), result)
    }
}
...
impl SrtSocket {
    ...
    pub fn get_linger(&self) -> Result<i32> {
        let mut linger = linger { l_onoff: 0, l_linger: 0 };
        let mut _optlen = mem::size_of::<i32>() as i32;
        let result = unsafe {
            srt::srt_getsockflag(
                self.id,
